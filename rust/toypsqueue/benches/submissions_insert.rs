...
pub async fn create_fake_submission_single_chunk(conn: &mut SqliteConnection, submission: Submission, chunks: Vec<Chunk>) {
    let _ = conn.transaction(|tx| Box::pin(async move {
        for chunk in chunks {
...
pub async fn create_fake_submission_multi_chunk(conn: &mut SqliteConnection, submission: Submission, chunks: Vec<Chunk>) {
    let _ = conn.transaction(|tx| Box::pin(async move {
...
pub async fn create_fake_submission_multi_transaction(conn: &mut SqliteConnection, submission: Submission, chunks: Vec<Chunk>) {
    let chunks_per_transaction = 10_000;
    for transaction_chunks in chunks.chunks(chunks_per_transaction).map(|c| c.to_vec()) {
        let _ = conn.transaction(|tx| Box::pin(async move {
            chunk::insert_many_chunks(&transaction_chunks, &mut **tx).await
        })).await;
    }
        ...
        group.bench_with_input(BenchmarkId::new("multi_chunk", size), &size, |b, size| {
            b.to_async(&runtime).iter_custom(|iters| async move {
                ...
                for _i in 0..iters {
                    black_box({
                        let (submission, chunks) = build_fake_submission(size);
                        let _ = create_fake_submission_multi_chunk(&mut db, submission, chunks).await;
                    })
                }
                start.elapsed()
            });
        });
        group.bench_with_input(BenchmarkId::new("multi_transaction", size), &size, |b, size| {
            b.to_async(&runtime).iter_custom(|iters| async move {
                let mut db = sqlx::SqliteConnection::connect(DATABASE_URL)
                    .await
                    .expect("Could not connect to sqlite DB");
                ...
                for _i in 0..iters {
                    black_box({
                        let (submission, chunks) = build_fake_submission(size);
                        let _ = create_fake_submission_multi_transaction(&mut db, submission, chunks).await;
                    })
                }
                start.elapsed()
            });
