...
use crate::server::interface::SubmissionStatusChangedSender;
...
impl ServerState {
    pub fn new(
        ...
        Self::new_with_status_sender(pool, notify_on_insert, max_submissions, None)
    }
    ...
    match submission::db::cancel_submission(submission_id, &mut conn).await {
        Ok(()) => {
            if let Some(sender) = &state.status_changed_sender {
                let _ = sender.send(submission_id);
            }
            Ok(())
        }
