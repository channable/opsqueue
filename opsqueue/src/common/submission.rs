...
fn snowflake_timestamp_ms(snowflake: u64) -> u64 {
...
where
    ...
    Tick: Fn(),
{
    let instance = SUBMISSION_SNOWFLAKE_INSTANCE;
    ...
    loop {
        ...
        let next = if now_ms_value < last_timestamp_ms {
            ...
            pack_submission_snowflake(now_ms_value, instance, 0)
        } else if last_sequence < SEQUENCE_MASK {
            pack_submission_snowflake(now_ms_value, instance, last_sequence + 1)
        } else {
            let next_ms = wait_until_next_millisecond_with(last_timestamp_ms, &now_ms, &tick_wait);
            pack_submission_snowflake(next_ms, instance, 0)
        };
...
/// Generate a new snowflake value with behavior matching `snowflaked`.
fn generate_submission_snowflake() -> u64 {
    generate_submission_snowflake_with(&LAST_SUBMISSION_ID, now_unix_ms, spin_loop)
}
